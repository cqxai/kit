...
/**
 * The shared store of published datasets. Declared by the page — it was a
 * build-time environment variable when this file lived in one application,
 * and an engine that two applications install cannot read either one's build.
 */
...
const bases = (): string[] => [declared, shared()].filter((b): b is string => !!b);
