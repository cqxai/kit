...
import type { Reply, Request } from '../workers/analyse.js';
import type { Commit, Dataset } from './types.js';
import type { RepoIndex } from './store.js';
 ...
 *
 * Made by the page rather than here: `new URL('./worker.ts', import.meta.url)`
 * is a literal the bundler rewrites, and it cannot rewrite one that lives in a
 * dependency. See Host.
 */
...
export async function liveIndex(repo: string): Promise<RepoIndex> {
  const commits = await fetchCommits(repo, policy.commits);
