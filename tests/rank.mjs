 ...
 *
 * The explorer's browser test proves the palette is wired up; it cannot prove
 * that the right thing is at the top, because what a real repository contains
 * changes under it. These are the orderings that have to hold whatever is
 * loaded, and they belong here now that the ranker does.
 *
 * Against `dist`, not `src`: what a consumer imports is the built artifact,
 * and the one bug this package has already shipped was visible only there.
 * Run `npm run build` first — `npm test` does.
 */
import { buildIndex, search } from '../dist/engine/index.js';
